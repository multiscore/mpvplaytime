...
2. (Only in Linux and macOS) Install `mpv` media player: `sudo apt install mpv` in Debian based distros or `brew install mpv` for the terminal command in macOS Homebrew (or `brew install --cask mpv` for the LaunchPad app).
